...
## Variance
```{r, Variance plot, fig.cap = "Barplot of Variancesf", echo = FALSE} 
...
ggboxplot(geologyFactors,x = "geology", y = "TDS", color = "geology", x.text.angle = 45, ylab = "TDS mg/L", font.legend = 6)
...
ggboxplot(geologyFactors,x = "geology", y = "TS", color = "geology", x.text.angle = 45, ylab = "TS mg/L", font.legend = 6)
```
...
ggboxplot(geologyFactors,x = "geology", y = "Cl", color = "geology", x.text.angle = 45, ylab = "Cl mg/L", font.legend = 6)
```
...
ggboxplot(wellGroups,x = "group", y = "TDS", color = "group", ylab = "TDS mg/L")
```
...
ggboxplot(wellGroups,x = "group", y = "TS", color = "group", ylab = "TS mg/L")
```
...
ggboxplot(wellGroups,x = "group", y = "Cl", color = "group", ylab = "Cl mg/L")
```
...
ggboxplot(geologyFactors,x = "geology", y = "altitude.ft", color = "geology", ylab = "Altitude in Feet", x.text.angle = 45, font.legend = 6)
```
...
# Conclusion
Considering all of the above analyses, I would say it is safe to say that Wells that have a Sand rock formation have different, specifically higher, levels of TDS, TS and Cl in them when compared to wells with different rock formations. It is also apparent that most of the wells that have a sand rock formation are in group 2, and are typically lower in elevation. This could indicate that elevation may be to blame, and sand is more likely in lower elevations. Further analyses (that I will do later on) will need to be performed to determine the role elevation plays. 
